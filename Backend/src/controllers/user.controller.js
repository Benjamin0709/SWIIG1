import User from "../models/user.model.js";

const createUser = async(req, res) => {
   
    try {

        const useEmail = await User.findOne({email: req.body.email})

        if(useEmail){
            throw new Error("Email is already in use!")
        }

        const newUser = new User(req.body);
        newUser.encriptarPassword(req.body.password);
        await newUser.save();
        
    
        res.status(201).json({
            success: true, 
            message: "User created successfully!", 
            info: newUser._id, 
            token: newUser.generateToken()
        })
            
    } catch (error) {
        res.json({success: false, message: error.message})
    }
}

const getUsers = async(req, res) => {
    try {
        const users = await User.find().select('-password -salt');
        // .populate('favoriteProducts')
        res.json({success: true, info: users })
    } catch (error) {
        res.json({success: false, message: error.message})
    }
}

const getProfile = async(req, res) => {
    try {
        const {id} = req.params;
        const getInfoUser = await User.findById(id).select("-password -salt")

        res.json({success: true, info: getInfoUser })
    } catch (error) {
        res.json({success: false, message: error.message})
    }
}

// Funciones actualizar y delete

const editUser = async(req, res) => {

    try {
        // throw new Error('error forzado')
        const {id} = req.params;
        const contain = req.body;

        const updateUser = await User.findByIdAndUpdate(id, contain, {new: true});

        res.json({success: true, msg: "User updated successfully!", updateUser})
    } catch (error) {
        res.status(500).json({success: false, message: error.message})
    }
}

const deleteUser =  async(req, res) => {
    try {
        // throw new Error('error forzado')
        const {id} = req.params;

        const destroyUser = await User.findByIdAndDelete(id);

        res.json({success: true, msg: "User deleted successfully!", destroyUser})
    } catch (error) {
        res.status(500).json({success: false, message: error.message})
    }
}

const loginUser = async(req, res) => {
    try {
        const {email, password} = req.body;
        const user = await User.findOne({email}); // buscando email en mongo atlas
    
        if(!user){
            throw new Error("User not found!") // no encontro el usuario
        } 

        const validarPassword = user.verificarEncriptacion(password, user.salt, user.password)

        if(!validarPassword){
            throw new Error('Email or password is incorrect!')
        }

        res.json({success: true, msg: "Login successful!", token: user.generateToken()})


    } catch (error) {
        res.status(500).json({success: false, message: error.message})
    }
}


const getVerifyUser = async(req, res) => {
    try {
        const {id} = req.auth;
        const getInfoUser = await User.findById(id).select("-password -salt")

        res.json({success: true, msg: `Information of: ${getInfoUser.email}`, info: getInfoUser })
    } catch (error) {
        res.json({success: false, message: error.message})
    }
}

export {
  createUser,
  getUsers,
  editUser,
  deleteUser,
  loginUser,
  getProfile,
  getVerifyUser
};